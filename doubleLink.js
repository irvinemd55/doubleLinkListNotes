...
// Node constructor similar to singly linked list constructor but with
// the new .previous property to allow for traversal both forward and back
function Node(data){
  this.data = data;
  this.previous = null;
  this.next = null;
...
dll.insertBefore(7,4);
// dll.traverse(function(node) {node.data = node.data + 10; });
// dll.traverseReverse(function(node) {node.data = node.data + 10; });
console.log('length is 7:', dll.length());
dll.print();
// console.log(dll);
// console.log(dll.findMiddle(dll));
console.log(dll.findnth(dll, 2));
// console.log(dll.findMiddleReverse(dll));
console.log(dll.findnthReverse(dll, 2));
